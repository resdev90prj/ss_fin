...
<p class="text-sm text-slate-600 mb-4">Gestao administrativa de usuarios com isolamento por login e escopo de visualizacao.</p>
...
<div class="bg-amber-100 border border-amber-200 text-amber-800 rounded p-3 mb-4 flex items-center justify-between gap-3">
  <span>Voce esta visualizando o sistema no escopo do usuario ID <?= (int)$scopedUserId ?>.</span>
  <form method="POST" action="index.php?route=users_clear_scope">
    <input type="hidden" name="_csrf" value="<?= e(csrf_token()) ?>">
    <button class="bg-amber-700 hover:bg-amber-600 text-white rounded px-3 py-1">Voltar para meu usuario</button>
  </form>
</div>
<?php endif; ?>

<form method="POST" action="index.php?route=users_store" class="bg-white p-4 rounded shadow mb-6 grid md:grid-cols-7 gap-3">
  <input type="hidden" name="_csrf" value="<?= e(csrf_token()) ?>">
  ...
  <input type="password" name="password" placeholder="Senha (min. 6)" minlength="6" required class="border rounded p-2">
  ...
  <button class="bg-slate-900 text-white rounded p-2 md:col-span-1">Criar acesso</button>
</form>
...
<table class="w-full text-sm">
  <thead class="bg-slate-200">
    <tr>
      ...
      <th class="p-2 text-center">Acoes</th>
    </tr>
  </thead>
  <tbody>
  <?php foreach ($users as $u): ?>
    ...
    <tr class="border-t align-top">
      ...
      <td class="p-2"><?= e($u['name']) ?><?= $isSelf ? ' <span class="text-xs text-slate-500">(voce)</span>' : '' ?></td>
      ...
      <td class="p-2 text-center space-y-2">
        ...
        <form method="POST" action="index.php?route=users_toggle_status" onsubmit="return confirm('Alterar status deste usuario?');">
          <input type="hidden" name="_csrf" value="<?= e(csrf_token()) ?>">
        ...
        <details>
          ...
          <form method="POST" action="index.php?route=users_update" class="mt-2 space-y-1 text-left">
            ...
            <button class="bg-blue-600 text-white rounded px-2 py-1 w-full">Salvar edicao</button>
          </form>
        </details>

        <details>
          <summary class="cursor-pointer text-blue-600">Alterar senha</summary>
          <form method="POST" action="index.php?route=users_reset_password" class="mt-2 space-y-1 text-left" onsubmit="return confirm('Alterar senha deste usuario?');">
            <input type="hidden" name="_csrf" value="<?= e(csrf_token()) ?>">
            <input type="hidden" name="id" value="<?= (int)$u['id'] ?>">
            <input type="password" name="new_password" placeholder="Nova senha" minlength="6" required class="border rounded p-1 w-full">
            <input type="password" name="confirm_password" placeholder="Confirmar nova senha" minlength="6" required class="border rounded p-1 w-full">
